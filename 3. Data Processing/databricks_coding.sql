...
SELECT
COALESCE (A.userID,B.userID) AS User_ID,
 A.UserID,
      A.Gender,
      A.Race,
      A.Age,
      ...
      B.RecordDate2,
      B.`Duration 2`
FROM `workspace`.`brightlearntv`.`user_profiles`AS A
FULL OUTER JOIN `workspace`.`brightlearntv`.`viewership2` AS B
ON A.UserID=B.UserID;

----Viewing the first 100 rows

select * from `workspace`.`brightlearntv`.`merged_table_1` limit 100;


---Counting the user ID there 10989 on count table

SELECT COUNT (UserID)
FROM `workspace`.`brightlearntv`.`merged_table_1`;


----Understanding the distinct province, race, gender
...
---Applying If Null to remove the Nulls in the table
...
SELECT
   ...
   ---Case statements on Age, Channel viewed,gender,race,province, time 
   CASE 
   ...
   CASE
      WHEN Province = 'Eastern_Cape' THEN 'Eastern_Cape'
   ...
   CASE
      WHEN Channel_2 = 'SuperSport_Blitz' THEN 'SuperSport_Blitz'
